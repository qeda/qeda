...
module.exports = (pattern, pitch, span1, span2, height, leadCount) ->
  pitch /= 100.0
  span1 /= 100.0
  span2 /= 100.0
  height /= 100.0
  ...
  y = -pitch * (housing.leadCount1/2 - 0.5)
  num = 1
  for i in [1..housing.leadCount1]
    pad1.name = num++
    pad1.y = y
  ...
  x = -pitch * (housing.leadCount2/2 - 0.5)
  pad2.y = padDistance2 / 2
  for i in [1..housing.leadCount2]
    pad2.name = num++
    pad2.x = x
    pattern.addPad pad2
    x += pitch
  ...
  y = -pitch*(housing.leadCount1/2 - 0.5) - padHeight1/2 - r - settings.clearance.padToSilk
